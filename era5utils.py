...
def extract_data(latt,lont,timt,data,datas):
    ...
    T = data01['t'].values * units.degK
    q = data01['q'].values 
    theta = potential_temperature(p, T)
    Td= dewpoint_from_relative_humidity(T, data01['r']*units.percent)
    ...
    q2m  = specific_humidity_from_dewpoint(sfcp*units.Pa,data02['d2m'].values*units.degK)
    theta2m = potential_temperature(sfcp*units.Pa, data02['t2m'].values*units.degK)
    td2m = data02['d2m'].values*units.degK
    t2m  = data02['t2m'].values*units.degK
    u10 = data02['u10'].values * units('m/s')
    v10 = data02['v10'].values * units('m/s')
    
    # Calculate Bunkers Storm Motion:
    [rightm,meanm,leftm]=bunkers_storm_motion(p[z>orog], u[z>orog], v[z>orog], z[z>orog]*units.meter)
    ...
    for i in range(0,len(p)):
        if calclow==0:
            if z[i]>orog:
                parcel_prof = mpcalc.parcel_profile(p[i:], T[i], Td[i]).to('degC')
...
#    mymap, mymin, mymax = Colormap(color_cols, color_vals)
    ...
    cf = ax.contourf(X, Y, cape, levels=levels, cmap=colormap, norm=norm, vmin = vmin,
